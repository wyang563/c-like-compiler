    ...
    fn emit_instruction(&mut self, instr: &Instr, func: &FunctionIR) {
        match &instr.kind {
            ...
            InstrKind::Assign { ty, src } => {
                ...
                let dest = self.value_operand(instr.results[0]);
                self.emit_instr(&format!("mov{} {}, {}", suffix, src_op, reg));
                self.emit_instr(&format!("mov{} {}, {}", suffix, reg, dest));
            }
        }
    }
    ...
    fn emit_terminator(&mut self, func_name: &str, term: &Terminator, func: &FunctionIR) {
        match term {
            ...
            Terminator::CBr {
                ...
                self.emit_instr(&format!("jne {}", self.block_label(func_name, *then_bb)));
